...
- `test:prod:installed-computer-use-parity` builds current local artifacts, verifies Computer Use failure shaping and timeline UI, checks `/Applications/pi-gui.app` extension surfacing, locked-use active-turn wiring, and locked-readiness status, then requires an unlocked active desktop for the installed background helper probe and real-auth live composer run. It blocks with `COMPUTER_USE_INSTALLED_PARITY_GATE_BLOCKED` instead of passing by skipped live coverage.
